"""EDU06 R06: captured parallel-rack gripper, dimensions in millimetres.
...
def rounded_rect(w,h,r,cx=0,cy=0):
    ...
    for x,y,start in [(cx+w/2-r,cy-h/2+r,-90),(cx+w/2-r,cy+h/2-r,0),
        ...
        points.extend((x+r*math.cos(math.radians(start+i*9)),
                       y+r*math.sin(math.radians(start+i*9))) for i in range(11))
    return points
...
def build_gripper(parent, tf, control, mats, HOME=None):
    ...
    motor=make_motor('AX','GRIP',fixed,Matrix.Identity(4),mats,attachment_depth=6,
                     front_fasteners=True,style='sculpted')
    ...
    def part(o,code,node=fixed,note='',orient=None):
        if code=='G01_Windowed_rear_palm':
            # Weld sub-micron Boolean intersection noise before triangulation;
            # keep nominal curves, bores and all functional datums unchanged.
    ...
    # One planar region with4mm frame and6mm bosses; no coplanar rib unions.
    from palm_profile import make_palm
    palm=make_palm(REAR_FACE,END_Y,rounded_rect,green)
    ...
    part(palm,'G01_Windowed_rear_palm',note='75 x123.7 mm open rear frame,4 mm ribs and6 mm local bolted bosses; rear datum -50.95. Four PCD48 phase-zero M3 interface holes. CentreØ26access plus20x10 rear AX connector passage at(0,-15); diagonal printed ribs connect lower interface boss to both motor ribs. Four AX cradle rear captive nuts accept front-inserted M3x45. Four frame-tie nut pockets open rearward.')
    ...
    for tag,node in [('R',right),('L',left)]:
        ...
        finger=L.poly('rounded finger body',rounded_rect(24,12,2,0,c),22,75,green)
        L.union(finger,L.poly('rounded finger root flange',rounded_rect(42,16,3,0,c),18,4,green))
        ...
        pocket=L.poly('_rounded finger rear lightening',rounded_rect(18,60.5,4,0,57.25),-(c+7),6.65)
        pocket.data.transform(L.R('X',90))
        L.boolean(finger,pocket)
        for x in (-16,16):
        ...
        cap=L.poly('rounded finger tip cap',rounded_rect(24,15,2,0,c-1.5),97,3,ivory)
    ...
    notes={
        'revision':'R06','units':'mm','control_property':'GRIP','home_gap_mm':gap_home,
        ...
        'rear_interface_screw':'4 x M3x12 from full6 mm palm seats; four captive nuts in J5 flange; wrist builder provides these references',
        ...
        'assembly_sequence':[
            ...
            'Insert rear palm frame-tie nuts and motor-cradle nuts before attaching palm to wrist. With AX absent, install four PCD48 interface M3x12 from front with a straight driver; the centreØ26 opening exposes J5 stock-horn M2 screws.',
    ...
    (Path(__file__).resolve().parent/'gripper-design-notes.json').write_text(json.dumps(notes,indent=2),encoding='utf-8')
