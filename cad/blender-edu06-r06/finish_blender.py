...
base=bpy.data.objects.get('B01_open_front_base_240x240')
...
if font_path.exists():
    title_font=bpy.data.fonts.load(str(font_path))
    title_font.pack()
    for name in ['Presentation title','Presentation subtitle']:
        if bpy.data.objects.get(name):bpy.data.objects[name].data.font=title_font
...
for old_scene in list(bpy.data.scenes):
    if old_scene.name.startswith('EDU06 R06 | Individual print parts'):
        for old_obj in list(old_scene.objects):bpy.data.objects.remove(old_obj,do_unlink=True)
        bpy.data.scenes.remove(old_scene)
layout=bpy.data.scenes.new('EDU06 R06 | Individual print parts')
...
hidden=[]
for o in scene.objects:
    upper=False;ancestor=o
    while ancestor:
        if (ancestor.name.startswith('J2 ') and 'DATUM' in ancestor.name) or ancestor.name.startswith(('B02_','B10_','B19_','Base lid M3x12 ','Deck flange M3x12 ','Pedestal to deck M3x25','Pedestal deck M3nut')):upper=True;break
        ancestor=ancestor.parent
    if upper or o.get('harness_id','').startswith(('H02_','H03_','H04_','H05_','H06_')):hidden.append(o);o.hide_render=True
camera.location=(-280,-490,720);target2=Vector((0,-23,35));camera.rotation_euler=(target2-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.ortho_scale=520
