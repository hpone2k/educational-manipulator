"""EDU06 R05: captured parallel-rack gripper, dimensions in millimetres.
...
def build_gripper(parent, tf, control, mats, HOME=None):
    ...
    palm=L.poly('rounded rear palm perimeter',rounded_rect(74,122.7,6),REAR_FACE,4,green)
    L.boolean(palm,L.poly('_rounded open perimeter',rounded_rect(66,114.7,3),REAR_FACE-2,8))
    L.union(palm,L.cyl('central access rim',17,4,(0,0,-48.95),mat=green))
    L.union(palm,L.poly('round-ended interface rib X',rounded_rect(58,10,5),REAR_FACE,4,green))
    L.union(palm,L.poly('round-ended interface rib Y',rounded_rect(10,58,5),REAR_FACE,4,green))
    for sx in (-1,1):
    ...
    notes={
        'revision':'R05','units':'mm','control_property':'GRIP','home_gap_mm':gap_home,
