...
def build_wrist(parent,tf,control,mats,home):
    ...
    union(fork,L.poly('_heel to motor flange rear rib',rounded_link((base_x,-23),(base_x,-43),4),motorcap-5,5,mats['green']))
    ...
    grip=build_gripper(j5,T(0,0,74.45),control,mats,home)
    for i,(x,y) in enumerate(pcd(4,24)):
        sp=ring('_palm screw spacer',3.5,1.7,1,29.5,mats['ivory']);part(sp,'W11_palm_screw_spacer_'+str(i),j5,T(x,y))
        bolt('J5 palm interface M3x12',j5,T(x,y,30.5),12,mats['steel'])
    metadata={'fixed_mount_to_J4_pitch_mm':D,'J4_pitch_to_J5_roll_horn_mm':forward,'J5_roll_to_gripper_horn_mm':74.45,
