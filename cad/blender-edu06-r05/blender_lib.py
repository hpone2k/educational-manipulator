...
def setup():
    ...
    scene=bpy.data.scenes.new('EDU06 R05 | Engineering assembly')
...
def rounded_profile(width,height,radius,centre=(0,0),steps=12):
    """Counterclockwise rounded rectangle, with exact nominal outside bounds."""
    r=min(radius,width/2,height/2);cx,cy=centre;pts=[]
    for x,y,start in [(width/2-r,height/2-r,0),(-width/2+r,height/2-r,90),(-width/2+r,-height/2+r,180),(width/2-r,-height/2+r,270)]:
        for i in range(steps+1):
            a=math.radians(start+90*i/steps);pts.append((cx+x+r*math.cos(a),cy+y+r*math.sin(a)))
    return pts

def hull_circles(circles,steps=40):
    """Convex perimeter of round mounting bosses: (x,y,radius)."""
    points=sorted(set((x+r*math.cos(2*math.pi*i/steps),y+r*math.sin(2*math.pi*i/steps)) for x,y,r in circles for i in range(steps)))
...
def joint(name,parent,tf,controller,prop,lo,hi,home):
    ...
    controller[prop]=float(home);controller.id_properties_ui(prop).update(min=lo,max=hi,soft_min=lo,soft_max=hi,description='Output angle in degrees')
...
def bolt(name,node,tf,length,mat,diam=3):
    ...
    reference(o,node,tf)
    o['fastener_kind']='screw';o['nominal_diameter_mm']=float(diam);o['under_head_length_mm']=float(length)
...
def nut(name,node,tf,mat,diam=3):
    ...
    reference(o,node,tf)
    o['fastener_kind']='nut';o['nominal_diameter_mm']=float(diam)
...
def export_parts():
    ...
    for o in PRINT:
        ...
        with (folder/(o['part_id']+'.stl')).open('wb') as f:
            f.write(b'EDU06 R05 millimetres prototype'.ljust(80,b' '));f.write(struct.pack('<I',len(bm.faces)))
