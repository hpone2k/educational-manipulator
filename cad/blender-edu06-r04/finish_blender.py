...
scene=next(s for s in bpy.data.scenes if s.name.startswith('EDU06 R04'))
...
control.id_properties_ui('J2').update(min=60.0,max=85.0,soft_min=60.0,soft_max=85.0,description='Shoulder output degrees;60deg lower operating limit from gravity screen')
j2=next(o for o in scene.objects if o.name.startswith('J2 ') and o.get('axis'))
j2['limits_deg']=[60.0,85.0]
...
for old_scene in list(bpy.data.scenes):
    if old_scene.name.startswith('EDU06 R04 | Individual print parts'):
        for old_obj in list(old_scene.objects):bpy.data.objects.remove(old_obj,do_unlink=True)
        bpy.data.scenes.remove(old_scene)
layout=bpy.data.scenes.new('EDU06 R04 | Individual print parts')
