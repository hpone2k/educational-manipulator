"""Build EDU06 R04 in a separate Blender process. All coordinates in mm.
...
HOME={'J1':0,'J2':65,'J3':55,'J4':0,'J5':0,'J6':0,'GRIP':45.0}
PAIRS=[];FASTENERS=[]
...
def arm(j1):
    ...
    _,j2,f2=reducer('S',j1,T(0,0,186)@R('X',90),'J2',5,1.25,HOME['J2'],(60,85))
    link('A01',j2,100,f2)
    ...
    j6,grip,pair=build_wrist(j3,T(123.75,0,z)@R('Y',90),control,M,HOME)
    PAIRS.append(pair)
...
def animate():
    ...
    poses=[(1,HOME),(60,{**HOME,'J1':28}),(110,{**HOME,'J1':-28}), (155,{**HOME,'J2':77,'J3':66}),(200,{**HOME,'J4':30,'J5':18}),(245,{**HOME,'J6':35,'GRIP':20.0}),(300,{**HOME,'J4':-25,'J5':-18,'GRIP':70.0}),(360,HOME)]
...
def presentation():
    ...
    L.text('Presentation subtitle','6 AXES + GEARED GRIPPER    |    R04 ENGINEERING PROTOTYPE',centre+up*212,5.5,M['text'],cam.rotation_euler,col=L.studio)
...
def run():
    ...
    try:
        ...
        notes=bpy.data.texts.new('START HERE — R04 status and controls');notes.write('EDU06 R04. Geometry in millimetres.\nSelect CONTROL; Custom Properties contain J1…J6 output angles and GRIP clear jaw gap in mm.\nTimeline has a 360-frame kinematic demonstration. Drivers enforce ratios J1 3:1 /J2 5:1 /J3 4:1 /J5 3:1 and opposed linear rack jaws.\nIndividual printable meshes: collection01. Purchased references: collection02, exclude from printing.\nThis file is an engineering prototype. Consult engineering-review.md and validation JSON files; no physical load test or full stress/thermal validation is claimed.\nPrint F01 fit coupon before committing. Filament, horn revision, actual screws and mounting details require physical measurement.\n')
        bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'EDU06_R04.blend'))
        say('SAVED native .blend. Render in a fresh process to evaluate all joint drivers.')
        (ROOT/'BUILD_COMPLETE.json').write_text(json.dumps({'ok':True,'printed_meshes':len(metrics),'blend':'EDU06_R04.blend'},indent=2));say('BUILD COMPLETE')
    except Exception:
