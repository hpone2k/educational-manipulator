"""Read-only independent Blender mass/kinematic/collision screening of EDU06 R04.
...
MOTOR_SCREEN = {
    ...
    'J5': {'motor': 'AX-12A', 'ratio': None, 'efficiency': .65, 'comparison_nm': .30},
...
def find_controls(keys=('J1', 'J2', 'J3', 'J4', 'J5', 'J6')):
...
def fingertip_midpoint(cache):
    # R04 records true flat-pad contact markers; do not substitute mesh extrema
...
def payload_tip_point():
    ...
    # Use the separate distal tip marker for the load case, otherwise R04's
...
def neutral_centreline_check(controller, joints, cache, home):
    neutral = {**home, 'J4': 0., 'J5': 0., 'J6': 0., 'GRIP': 45.}
    set_pose(controller, neutral)
    j4_matrix = world_matrix(joints['J4'])
    origin = j4_matrix.translation
    axis = (j4_matrix.to_3x3() @ Vector((0, 0, 1))).normalized()
    j5_matrix = world_matrix(joints['J5'])
    j6_matrix = world_matrix(joints['J6'])
    j5_axis = (j5_matrix.to_3x3() @ Vector((0, 0, 1))).normalized()
    j6_axis = (j6_matrix.to_3x3() @ Vector((0, 0, 1))).normalized()
    ...
    points = {'J5_pitch_origin': j5_matrix.translation,
              'J6_roll_origin': j6_matrix.translation,
    ...
    axial_stations = {name: float((point - origin).dot(axis)) for name, point in points.items()}
    axes = {'J4_dot_J6': float(axis.dot(j6_axis)),
            'J4_dot_tool': float(axis.dot(tool_axis)),
            'J4_dot_J5_pitch': float(axis.dot(j5_axis))}
    passed = (max(distances.values()) <= .05 and
              axes['J4_dot_J6'] >= 1 - 1e-5 and axes['J4_dot_tool'] >= 1 - 1e-5 and
              abs(axes['J4_dot_J5_pitch']) <= 1e-5 and
              axial_stations['J5_pitch_origin'] > 0 and
              axial_stations['J6_roll_origin'] > axial_stations['J5_pitch_origin'] and
              axial_stations['fixed_tool_tip_marker'] > axial_stations['J6_roll_origin'])
    result = {'pass': passed, 'neutral_controls': neutral,
              'J4_axis_origin_world_mm': list(origin), 'J4_axis_unit_world': list(axis),
              'distance_from_J4_axis_mm': distances, 'axial_stations_from_J4_mm': axial_stations,
              'axis_dot_products': axes,
              'position_tolerance_mm': .05,
              'scope': 'Verifies the neutral tool path is straight and centred while J5 is transverse; it does not imply the wrist remains collinear when pitch is commanded.'}
...
def audit_controls(controller, joints, cache, home, limits, gear_specs):
    ...
    for key, bounds in limits.items():
        ...
        ax_span = None
        if key in ('J1', 'J4', 'J5', 'J6', 'GRIP'):
            field = ('actual_grip_pinion_degrees' if key == 'GRIP' else
                     'actual_pinion_degrees' if key in ('J1', 'J5') else 'actual_output_degrees')
...
def canonical_rigid_group(obj):
    """Nearest actuated transform owns a body; includes translating R04 jaws."""
    current = obj
    while current is not None:
...
def audit_wrist_debug(args):
    ...
    controller, joints = find_controls(('J4', 'J5', 'J6'))
...
def main():
    ...
    parser.add_argument('--blend', type=Path, default=ROOT / 'EDU06_R04.blend')
    ...
    output = {
        'audit': 'R04_independent_Blender_world_mesh_mass_and_collision_screen_v2',
        ...
        'collision_classifier_reference_checks': {
            'passed': True,
            'cases': 'Inside/outside cube, hollow sleeve bore/wall, four translated/rotated sleeves, and four rotated thin solids with a positive interior sample and negative samples 0.25 mm beyond each face',
        ...
        'limitations': [
            ...
            'Collision rigid groups use the nearest ancestor with an actuated transform, including translating gripper jaws. Fixed DATUM transforms inherit their parent body. Pairs on the same moving body and all FIXED-to-FIXED pairs are ignored; self-assembly errors within those bodies are not checked.',
