"""Build EDU06 R03 in a separate Blender process. All coordinates in mm.
...
HOME={'J1':0,'J2':65,'J3':55,'J4':0,'J5':0,'J6':0,'GRIP':16}
PAIRS=[];FASTENERS=[]
...
def base():
    ...
    panel=add_part(front,'B03_service_front_panel',tf=T(28,-140,3)@R('X',90),note='Removable front panel; shown lying in front. Assembly transform: translation(0,-93.5,5) with no rotation.',orient=[90,0,0])
...
def reducer(prefix,parent,tf,prop,ratio,module,home,limits):
    teeth=20*ratio;C=module*(20+teeth)/2;face=12 if ratio==4 else 10
    bridge_pts=[(-30,-25),(-30,15)] if prop=='J2' else [(0,-30),(0,30)]
    ...
    for side,z in [('rear',-25),('front',19)]:
        ...
        if side=='rear':
            union(cheek,cube('_web',(C+26,18,6),(-C/2,-27,z+3),bevel=2))
            union(cheek,cube('_motor flange',(62,68 if prop=='J2' else 86,6),(-C-6,-12 if prop=='J2' else -5,z+3),bevel=2))
        ...
        if prop=='J2':
            web=poly('_pedestal web',[(-32,-63),(33,-63),(24,-10),(-24,-10)],z,6,M['green']);union(cheek,web)
            holes(cheek,pcd(4,22,math.pi/4),z+3)
    ...
    if teeth>=60:
        for x,y in pcd(8,teeth*module*.35):hole(wheel,(x,y,37),13 if teeth>=80 else 9,face+8)
    ...
    motor=make_motor('XM',prefix+'_XM',station,T(-C,0,31.5),M,attachment_depth=15.35)
    ...
    hz=31.5+face+2
    outline=[(-36,-31),(-36,21),(10,10),(10,-10)] if prop=='J2' else [(-8,-36),(-8,36),(8,36),(8,-36)]
...
def link(prefix,node,length,outface,distal=True):
    ...
    beam=cube('_box beam',(length-30,36,24),((length+6)/2,0,outface+12),M['green'],3)
    boolean(beam,cube('_hollow',(length-34,29,17),((length+6)/2,0,outface+12),bevel=2))
    ...
    for x in [length*.43,length*.67]:
        boolean(root,cube('_lightening window',(14,55,9),(x,0,outface+12),bevel=3))
    add_part(root,prefix+'_hollow_ribbed_link',node,note=f'{length} mm pivot spacing; closed corner rails and 3.5 mm nominal walls. Orient link flat with layer paths across root.')
...
def wrist_carrier(prefix,parent,sign=1,shift=0,rise=0):
    # Compact L carrier. Rear-cap mounting plane at x=-20 (or mirrored +20).
    b=cube('_carrier base',(54+abs(shift),70,4),(shift/2,-13,9),M['green'],1.3)
    hole(b,(0,0,9),9,8);holes(b,pcd(4,17,math.pi/4),9)
    for x,y in pcd(4,8):hole(b,(x,y,8.1),4.5,2.4)
    boolean(b,cube('_weight window',(29,16,10),(3,-32,9),bevel=2))
    wall=cube('_carrier upright',(4,70,55+rise),(-22*sign+shift,-13,35.5+rise/2),M['green'],1.3);union(b,wall)
    for px in [-21,21]:
        for py in [-42.5,15.5]:hole(b,(-22*sign+shift,py,35+rise-px*sign),3.4,14,'X')
    boolean(b,cube('_motor ventilation',(14,46,29+rise),(-22*sign+shift,-13,35+rise/2),bevel=3))
    add_part(b,prefix+'_printed_wrist_yoke',parent,note='Bolted to preceding AX output and next AX rear cradle on shared four-hole pattern. No metal bracket.')
    return b

def output_flange(prefix,j):
    o=ring('_AX flange',23,4,4,3,M['ivory']);union(o,ring('_standoff boss',11,4,3.2,0,M['ivory']))
    holes(o,pcd(4,8),3.5,2.3);holes(o,pcd(4,17,math.pi/4),5)
    add_part(o,prefix+'_AX_horn_adapter',j,note='AX 16PCD M2×10 through7mm =>3mm engagement. Raised flange leaves room for M3 nuts above cradle bezel.')
    screw_pattern(prefix+' M2 horn ',j,T(z=7),pcd(4,8),10,2)
    for i,(x,y) in enumerate(pcd(4,17,math.pi/4)):
        sp=ring('_washer',3.5,1.7,1.5,11,M['ivory']);add_part(sp,prefix+f'_carrier_screw_spacer_{i}',j,T(x,y))
        nut(prefix+' carrier nut',j,T(x,y,.6),M['steel'])
    screw_pattern(prefix+' carrier M3 ',j,T(z=12.5),pcd(4,17,math.pi/4),12)

def gripper(parent,tf):
    say('Building opposite-rotation geared gripper with printed journals')
    fixed=empty('GRIPPER fixed palm datum',parent,tf)
    motor=make_motor('AX','GRIP_AX',fixed,T(0,-15,0),M,attachment_depth=12)
    # Palm rear plate captures both the motor rear cap and the idler support column.
    palm=cube('_palm',(56,92,6),(0,-17,-47.95),M['green'],2)
    for x in [-21,21]:
        for y in [-57.5,.5]:hole(palm,(x,y,-47.95),3.4,10)
    boolean(palm,cube('_motor opening',(26,43,12),(0,-29,-47.95),bevel=3))
    hole(palm,(0,15,-47.95),3.4,12)
    hexhole(palm,(0,15,-49.55),5.8,2.8)
    add_part(palm,'G01_printed_palm',fixed,note='Fixed palm; four M3 rear-cradle fasteners plus matching tool carrier holes.')
    # Idler tower, two front bridge columns. All polymer.
    idler=ring('_idler tower',10,5.25,42.95,-44.95,M['green']);add_part(idler,'G02_idler_pivot_tower',fixed,T(0,15),note='Ø10.5 bore for printed Ø10 idler journal. Bolt preload passes through separate stationary sleeve.')
    for i,y in enumerate([-30,30]):
        col=ring('_bridge column',4.5,1.7,57.95,-44.95,M['green']);add_part(col,f'G03_bridge_column_{i}',fixed,T(-18,y))
        hole(palm,(-18,y,-47.95),3.4,10);hexhole(palm,(-18,y,-49.55),5.8,2.8)
        spacer=ring('_bridge screw spacer',3.5,1.7,2.15,17,M['ivory']);add_part(spacer,f'G10_bridge_screw_spacer_{i}',fixed,T(-18,y))
        bolt('Gripper bridge M3x70',fixed,T(-18,y,19.15),70,M['steel']);nut('Gripper bridge M3nut',fixed,T(-18,y,-50.75),M['steel'])
    bridge=cube('_bridge',(33,76,4),(-7.5,0,15),M['green'],1)
    for y in [-15,15]:
        union(bridge,cyl('_boss',8,6,(0,y,12)))
        hole(bridge,(0,y,12),8.5 if y<0 else 10.5,12)
    holes(bridge,[(-18,-30),(-18,30)],15)
    boolean(bridge,cube('_bridge window',(15,11,12),(-16,0,15),bevel=2))
    add_part(bridge,'G04_front_pivot_bridge',fixed,note='Bridge retains printed shafts; sleeve running bore8.5. Install after horn screws.')
    control['GRIP']=HOME['GRIP'];control.id_properties_ui('GRIP').update(min=0,max=24,description='Symmetric jaw opening angle, degrees')
    for lower,y,phase,sign in [(True,-15,99,-1),(False,15,90,1)]:
        node=empty('GRIP lower driven jaw' if lower else 'GRIP upper idler jaw',fixed,T(0,y))
        L.drive(node,control,'GRIP',f'{sign}*q*pi/180')
        if lower:rotating_horn(motor,node)
        jaw=gear('_jaw gear',20,1.5,6,0,phase)
        points=[(8,-6),(23,-4),(40,9),(59,9),(59,14),(37,14),(20,2),(8,6)]
        if not lower:points=[(x,-y) for x,y in points]
        union(jaw,poly('_jaw finger',points,0,6,M['ivory']))
        union(jaw,ring('_front journal',4 if lower else 5,1.7 if lower else 3.2,11.5,6,M['ivory']))
        if lower:
            holes(jaw,pcd(4,8),3,2.3)
            for x,y0 in pcd(4,8):hole(jaw,(x,y0,5),4.3,2)
            screw_pattern('Gripper AX horn M2 ',node,T(z=4),pcd(4,8),6,2)
        else:
            union(jaw,ring('_rear journal',5,3.2,44,-44,M['ivory']))
            hole(jaw,(0,0,3),6.4,8)
        add_part(jaw,'G05_driven_gear_jaw' if lower else 'G06_idler_gear_jaw',node,note='20T m1.5 gears with integrated pivoting jaws;30mm shaft spacing. Not a parallel-jaw gripper.')
    sleeve=ring('_stationary compression sleeve',3,1.7,62.95,-44.95,M['ivory'])
    add_part(sleeve,'G07_stationary_bolt_sleeve',fixed,T(0,15),note='Printed6OD/3.4ID sleeve prevents bolt preload locking the idler jaw; rotating bore6.4 gives0.2 radial gap.')
    cap=ring('_idler retainer',7,1.7,2,18,M['green']);add_part(cap,'G08_idler_retaining_cap',fixed,T(0,15),note='0.5mm axial gap above rotating journal,0.95mm rear gap; cap loads stationary sleeve.')
    spacer=ring('_idler head spacer',3.5,1.7,3.95,20,M['ivory']);add_part(spacer,'G09_idler_screw_spacer',fixed,T(0,15))
    bolt('Idler M3x75',fixed,T(0,15,23.95),75,M['steel']);nut('Idler captiveM3',fixed,T(0,15,-50.75),M['steel'])
    PAIRS.append({'name':'GRIP',**pair_spec(20,20,1.5)})
    return fixed

def arm(j1):
    say('Building real 4:1 shoulder and 3:1 elbow stages')
    _,j2,f2=reducer('S',j1,T(0,0,186)@R('X',90),'J2',4,1.25,HOME['J2'],(45,85))
    link('A01',j2,100,f2)
    ...
    _,j3,f3=reducer('E',j2,elbow_tf,'J3',3,1.25,HOME['J3'],(30,75))
    ...
    # Roll unit behind its horn fits after the forearm, with matching rear-cap mount.
    end=cube('_roll bulkhead',(6,70,54),(75.8,-13,z),M['green'],2)
    for x in [-21,21]:
        for y in [-42.5,15.5]:hole(end,(75.8,y,z-x),3.4,12,'X')
    boolean(end,cube('_wire aperture',(12,20,20),(75.8,-13,z),bevel=2))
    forearm=next(o for o in L.PRINT if o.name=='A03_hollow_ribbed_link');union(forearm,end);L.finish(forearm)
    forearm['description']='85mm hollow forearm with integral AX roll bulkhead, sharing42×58 M3 rear mounting pattern.'
    st4,j4=L.joint('J4 • forearm roll',j3,T(123.75,0,z)@R('Y',90),control,'J4',-50,50,HOME['J4'])
    motor=make_motor('AX','J4_AX',st4,Matrix.Identity(4),M,attachment_depth=6);rotating_horn(motor,j4);output_flange('W02',j4);wrist_carrier('W03',j4,1,-30,30)
    st5,j5=L.joint('J5 • wrist pitch',j4,T(-5.05,0,65)@R('Y',90),control,'J5',-30,30,HOME['J5'])
    motor=make_motor('AX','J5_AX',st5,Matrix.Identity(4),M,attachment_depth=4);rotating_horn(motor,j5);output_flange('W04',j5);wrist_carrier('W05',j5,-1)
    st6,j6=L.joint('J6 • tool roll',j5,T(-24.95,0,35)@R('Y',-90),control,'J6',-50,50,HOME['J6'])
    motor=make_motor('AX','J6_AX',st6,Matrix.Identity(4),M,attachment_depth=4);rotating_horn(motor,j6);output_flange('W06',j6)
    # Tool flange to transverse gripper palm. Extended web closes the mechanical connection.
    adapter=cube('_tool carrier',(64,88,4),(4,-19,9),M['green'],1.2);holes(adapter,pcd(4,17,math.pi/4),9)
    for x,y in pcd(4,8):hole(adapter,(x,y,8.1),4.5,2.4)
    boolean(adapter,cube('_base lightening',(27,24,10),(-5,-40,9),bevel=3))
    union(adapter,cube('_tool spine',(6,70,64),(31.95,-28,41),M['green'],1.5))
    boolean(adapter,cube('_spine lightening',(12,40,29),(31.95,-28,45),bevel=3))
    for gy in [-57.5,.5]:
        for gx in [-21,21]:hole(adapter,(31.95,gy,45+gx),3.4,12,'X')
    add_part(adapter,'W07_gripper_mount',j6,note='Printed carrier attaches gripper palm to tool-roll horn adapter. Prototype interface under review.')
    grip=gripper(j6,T(-22,0,45)@R('Y',-90))
...
def animate():
    ...
    poses=[(1,HOME),(60,{**HOME,'J1':28}),(110,{**HOME,'J1':-28}), (155,{**HOME,'J2':77,'J3':66}),(200,{**HOME,'J4':30,'J5':18}),(245,{**HOME,'J6':35,'GRIP':3}),(300,{**HOME,'J4':-25,'J5':-18,'GRIP':22}),(360,HOME)]
    for frame,pose in poses:
        for key,value in pose.items():control[key]=value;control.keyframe_insert(data_path='["'+key+'"]',frame=frame)
...
def presentation():
    ...
    L.text('Presentation subtitle','6 AXES + GEARED GRIPPER    |    R03 ENGINEERING PROTOTYPE',centre+up*212,5.5,M['text'],cam.rotation_euler,col=L.studio)
    L.text('Presentation note','PRINTED STRUCTURE • PRINTED PLAIN BEARINGS • REAL MOTOR ENVELOPES',centre-up*240,5.1,M['text'],cam.rotation_euler,col=L.studio)
...
def run():
    try:
        ...
        metrics=L.export_parts();(ROOT/'gear-pairs.json').write_text(json.dumps(PAIRS,indent=2));(ROOT/'fasteners.json').write_text(json.dumps(FASTENERS,indent=2))
        ...
        (ROOT/'BUILD_COMPLETE.json').write_text(json.dumps({'ok':True,'printed_meshes':len(metrics),'blend':'EDU06_R03.blend'},indent=2));say('BUILD COMPLETE')
    except Exception:
